...
public class LikeablePersonController {
    ...
    @PreAuthorize("isAuthenticated()")
    @GetMapping("/toList")
    public String showToList(@RequestParam(required = false) String gender, Model model) {
        InstaMember instaMember = rq.getMember().getInstaMember();
        ...
        if (instaMember != null) {
            ...
            // gender 파라미터를 사용하여 필터링
            // gender에 따라 필터링되어 toList에서 표시되도록
            if (gender != null && !gender.isEmpty()) {
                likeablePeople = likeablePeople.stream()
                        .filter(p -> gender.equals(p.getToInstaMember().getGender()))
